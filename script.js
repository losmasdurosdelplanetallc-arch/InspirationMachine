const translations = {
  en: {
    ...
    rhymeNoMatches: "No verified matches in the offline library yet. Try another word.",
  ...
  es: {
    ...
    rhymeNoMatches: "Aún no hay rimas verificadas para esa palabra. Prueba otra.",
...
const rhymeFamilies = {
  en: [
    ...
    rhymeFamily("day|say|way|stay|play|gray|away|delay|they", "face|made|rain|change|late|safe|awake", "daily|date|shade|stage|save|same|fade"),
    rhymeFamily("rain|pain|chain|train|gain|lane|main|remain|reign|rein", "name|change|again|dream|time|flame|home", "rainfall|raindrop|range|ran|ring|runway|rainy"),
    ...
    rhymeFamily("fire|higher|wire|desire|choir|inspire|entire|acquire|liar", "tired|wild|life|rise|night|alive|skyline", "fear|fair|far|flare|fiery|fighter|final"),
    ...
    rhymeFamily("blue|you|true|through|new|due|view|crew|too|blew|do|shoe", "room|moon|move|loose|truth|youth", "blow|below|glue|clue|blur|bloom|blurry"),
    rhymeFamily("name|same|flame|game|frame|blame|shame|claim|aim", "rain|change|day|face|again|time|remain", "numb|named|naming|nominee|nameplate|nameless|number"),
    ...
    rhymeFamily("soul|whole|role|goal|coal|bowl|control|patrol|sole", "gold|home|alone|low|road|old|hope", "sorrow|sale|seal|soil|soulful|solar|solid"),
    ...
    rhymeFamily("air|care|dare|fair|hair|pair|share|stare|heir", "heart|hard|there|wear|prayer|fear|near", "airless|ear|are|airy|arrow|error|area"),
    ...
    rhymeFamily("face|grace|place|space|trace|case|base|embrace|phase", "fade|safe|change|day|late|rain|name", "facial|fast|feast|voice|fierce|facing|faces"),
    rhymeFamily("deep|keep|sleep|cheap|weep|steep|sweep|creep|leap", "dream|free|leave|need|believe|beneath|peace", "dip|drip|drop|deeper|depth|tip|deepen"),
    rhymeFamily("mind|find|kind|blind|grind|behind|remind|signed|mined", "time|night|life|inside|light|eyes|mine", "midnight|mend|meant|mood|mindless|winding|memory"),
...
function renderTools() {
  $("#toolGrid").innerHTML = toolDefs.map(tool => {
    ...
    return `<button class="tool-card ${activeTool === tool.id ? "active" : ""}" data-tool="${tool.id}" style="--tool-color:${tool.color}" type="button">
...
function challengeSummary(mission=currentMission) {
  ...
  const words = localized(mission.words);
  const constraint = localized(mission.constraint).replace(/[.!?]+$/,"");
  const mood = localized(mission.mood);
  const tempo = localized(mission.tempo);
  const additional = localized(mission.additional);
  const sentences = [];

  if (language === "es") {
    let opening = genre ? `Escribe una canción de ${genre}` : "Escribe una canción";
    if (perspective) opening += ` con la voz de ${lowerFirst(perspective)}`;
    sentences.push(`${opening}.`);
    if (emotion) sentences.push(`Haz que esta emoción guíe la canción: ${lowerFirst(emotion)}.`);
    if (setting) sentences.push(`Construye la escena principal alrededor de esta situación: ${lowerFirst(setting)}.`);

    if (!perspective && !emotion && !setting) {
      if (words) sentences.push(`Usa ${words} como puntos de partida para la letra.`);
      if (constraint) sentences.push(`Sigue esta regla: ${lowerFirst(constraint)}.`);
      if (tempo || additional) sentences.push(`Construye el sonido${tempo ? ` a ${tempo}` : ""}${additional ? ` con ${lowerFirst(additional)}` : ""}.`);
      if (mood) sentences.push(`Visualmente, apunta a ${lowerFirst(mood)}.`);
    }
  } else {
    let opening = genre ? `Write ${/^[aeiou]/i.test(genre) ? "an" : "a"} ${genre} song` : "Write a song";
    if (perspective) opening += ` in the voice of ${lowerFirst(perspective)}`;
    sentences.push(`${opening}.`);
    if (emotion) sentences.push(`Let this emotion guide the song: ${lowerFirst(emotion)}.`);
    if (setting) sentences.push(`Build the main scene around this situation: ${lowerFirst(setting)}.`);

    if (!perspective && !emotion && !setting) {
      if (words) sentences.push(`Use ${words} as lyrical anchors.`);
      if (constraint) sentences.push(`Follow this rule: ${lowerFirst(constraint)}.`);
      if (tempo || additional) sentences.push(`Build the sound${tempo ? ` at ${tempo}` : ""}${additional ? ` with ${lowerFirst(additional)}` : ""}.`);
      if (mood) sentences.push(`Visually, aim for ${lowerFirst(mood)}.`);
    }
  }
...
function generateTool(id) {
  activeTool = id;
  const tool = toolDefs.find(item => item.id === id);
  $$(".tool-card").forEach(button => button.classList.toggle("active", button.dataset.tool === id));
  const output = $("#toolOutput");
  output.style.setProperty("--active-tool-color", tool?.color || "#9a5cff");
  if (id === "rhyme") {
    output.classList.remove("has-result");
    output.classList.add("is-rhyme");
    renderRhymeForm();
    return;
  }
  const value = freshToolValue(id);
  if (!value) { showToast(translations[language].toolExhausted); return; }
  output.classList.remove("is-rhyme");
  output.classList.add("has-result");
  output.innerHTML = `<span>✦</span><div class="tool-generated-copy"><small>${tool[language][0]}</small><p>${value}</p></div>`;
...
function findRhymes(word) {
  const key = normalizeWord(word);
  const families = rhymeFamilies[language];
  const family = families.find(group => group.perfect.some(candidate => normalizeWord(candidate) === key));
  ...
  // For words without their own exact family, only return relationships that were
  // explicitly curated in the reverse direction. Never guess from spelling alone.
  const closeFamilies = families.filter(group => group.close.some(candidate => normalizeWord(candidate) === key));
  const phoneticFamilies = families.filter(group => group.phonetic.some(candidate => normalizeWord(candidate) === key));
  const close = uniqueRhymes(closeFamilies.flatMap(group => group.perfect), key).slice(0,12);
  const closeKeys = new Set(close.map(normalizeWord));
  const phonetic = uniqueRhymes(phoneticFamilies.flatMap(group => group.perfect), key, closeKeys).slice(0,12);
  return {perfect:[], close, phonetic};
}
