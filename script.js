...
function expandLibraries() {
  ...
  const places = pairs(
    ["An empty train station","A rooftop","A motel hallway","A closed diner","A hospital parking lot","A childhood bedroom","A recording studio","A corner store","An airport gate","A beach"],
    ["Una estación de tren vacía","Una azotea","El pasillo de un motel","Un restaurante cerrado","El estacionamiento de un hospital","Un dormitorio de infancia","Un estudio de grabación","Una tienda de la esquina","Una puerta de aeropuerto","Una playa"]);
  const conditions = pairs(
    ["at 2 AM","during a blackout","after the last customer leaves","at sunrise","in heavy rain","on the final day of summer","while everyone is asleep","after a missed call","during a citywide celebration","one hour before leaving"],
    ["a las 2 AM","durante un apagón","después de que se va el último cliente","al amanecer","bajo lluvia fuerte","el último día del verano","mientras todos duermen","después de una llamada perdida","durante una celebración en toda la ciudad","una hora antes de partir"]);
  fillPairedPool(data.setting, crossPairs(places, conditions, (a,b)=>`${a} ${b}`, (a,b)=>`${a} ${b}`));

  const narrators = pairs(
    ["Your future self","The person who left","Your childhood best friend","A stranger","The city","Your shadow","An old photograph","The family home","An unanswered voicemail","The person who stayed"],
    ["Tu yo del futuro","La persona que se fue","Tu mejor amistad de la infancia","Un extraño","La ciudad","Tu sombra","Una foto antigua","La casa familiar","Un mensaje de voz sin respuesta","La persona que se quedó"]);
  const narratorFrames = pairs(
    ["telling the truth for once","five years later","in an unsent letter","after the last phone call","watching from across the street","remembering it differently","trying not to apologize","before anyone knew","with nothing left to prove","speaking to one specific person"],
    ["diciendo la verdad por primera vez","cinco años después","en una carta que nunca envió","después de la última llamada","mirando desde la otra acera","recordándolo de otra manera","intentando no disculparse","antes de que alguien lo supiera","sin nada que demostrar","hablándole a una sola persona"]);
  ...
  const twistSources = pairs(
    ["The final line","The bridge","The second verse","The last chorus","A background vocal","The title","The instrumental break","The opening image","The narrator","The final chord"],
    ["La última línea","El puente","El segundo verso","El último coro","Una voz de fondo","El título","El interludio instrumental","La imagen inicial","Quien narra","El acorde final"]);
  const twistResults = pairs(
    ["reveals the narrator was lying","changes who the song is about","takes place ten years later","answers the opening question","shows the memory never happened","switches to the other person's point of view","turns the apology into a goodbye","reveals both characters are the same person","reframes the chorus as a warning","proves the title means the opposite"],
    ["revela que quien narra estaba mintiendo","cambia de quién trata la canción","ocurre diez años después","responde la pregunta inicial","muestra que el recuerdo nunca ocurrió","cambia al punto de vista de la otra persona","convierte la disculpa en una despedida","revela que ambos personajes son la misma persona","convierte el coro en una advertencia","demuestra que el título significa lo contrario"]);
  ...
  const lyricSubjects = pairs(
    ["I left","You kept","We found","The city hid","My shadow carried","Your silence broke","The radio played","The morning found","Nobody saw","Tonight I heard"],
    ["Dejé","Guardaste","Encontramos","La ciudad escondió","Mi sombra cargó","Tu silencio rompió","La radio tocó","La mañana encontró","Nadie vio","Esta noche escuché"]);
  const lyricObjects = pairs(
    ["your name in the static.","the truth under the passenger seat.","one last chance beside the door.","our summer inside an old receipt.","a goodbye in the kitchen light.","tomorrow behind the motel sign.","your promise on the wrong station.","my courage at the final red light.","the answer in a half-empty glass.","our old song through the apartment wall."],
    ["tu nombre en la estática.","la verdad debajo del asiento del pasajero.","una última oportunidad junto a la puerta.","nuestro verano dentro de un recibo viejo.","un adiós bajo la luz de la cocina.","el mañana detrás del letrero del motel.","tu promesa en la estación equivocada.","mi valor en la última luz roja.","la respuesta en un vaso medio vacío.","nuestra vieja canción a través de la pared."]);
